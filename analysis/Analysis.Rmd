...
set.seed(1)
data <- dat1[, c(3,4,6:17)] %>% mutate(id=row_number())
...
LdaModelForlikes = lda(likesQuartile9 ~ category + timePeriod + definition + caption + madeForKids + duration, data = train)
# Make predictions for trainset
...
# comments9
LdaModelForcomments = lda(commentsQuartile9 ~ category + timePeriod + definition + caption + madeForKids + duration, data = train)
# Make predictions for trainset
...
```{r}
tree = rpart(viewsQuartile3 ~ category+ timePeriod+ definition+ caption+ madeForKids+ + duration, data = train, method = "class")
printcp(tree)
plotcp(tree)
...
rf1=randomForest(viewsQuartile3 ~ category+ timePeriod+ definition+ caption+ madeForKids+ + duration, data = train, mtry=3, ntree=1000, nodesize = 1)
rf1

bag_test_pred1 <- predict(rf1, newdata = test)
bag_test_err1 <- mean(bag_test_pred1!=test$viewsQuartile3)
bag_test_err1
table(bag_test_pred1, test$viewsQuartile3)
